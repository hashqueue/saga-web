        ...
        <a-col :span="18">
          <a-tabs v-model:activeKey="contentActiveKey">
            <a-tab-pane key="detail" tab="详情">
              ...
              >
                ...
                <a-row :gutter="24">
                  ...
                  <a-col :span="12" v-if="createUpdateForm.work_item_type === 3">
                  ...
                  <a-col :span="12" v-if="createUpdateForm.work_item_type === 2">
                    <a-form-item
                      name="deadline"
                      label="截止日期"
                      v-if="createUpdateForm.work_item_type === 2"
                    >
                      <a-date-picker
                        placeholder="请选择截止日期"
            ...
            <a-tab-pane key="file" tab="文件" force-render>
              ...
              <a-list :loading="initLoading" item-layout="horizontal" :data-source="userFileList">
                <template #renderItem="{ item }">
                  <a-list-item>
                    ...
                    <a-skeleton avatar :title="false" :loading="!!item.loading" active>
                      <a-list-item-meta
                        :description="`${item.size} 来自 ${item.created_by} ${item.creator_name}`"
                      >
                        <template #title>
                          <p>
          ...
          <a-row :gutter="24" class="height-100">
            ...
            <a-col :span="22" class="height-100">
              <a-tabs v-model:activeKey="activityActiveKey">
                <a-tab-pane key="comment" tab="评论">
                  ...
                  >
                    <template #renderItem="{ item }">
                      <a-list-item>
                        <a-comment
                          :author="`${item.created_by} ${item.creator_name}`"
                ...
                <a-tab-pane key="changelog" tab="变更记录">
                  <a-list
...
import { createComment, getCommentList } from '@/apis/pm/comment'
  ...
  createUpdateFormRef.value
    ...
    .then((values) => {
      ...
      updateWorkItem(props.workItemId, values).then(() => {
        ...
        contentActiveKey.value = 'detail'
        activityActiveKey.value = 'comment'
        comments.value = []
        commentValue.value = ''
        changelogs.value = []
      })
